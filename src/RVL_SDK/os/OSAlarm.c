...
void OSSetAbsAlarm(OSAlarm* alarm, OSTime time, OSAlarmHandler handler) {
    ...
    ASSERTMSGLINE(382, handler, "OSSetAbsAlarm(): null handler was specified.");
    enabled = OSDisableInterrupts();
    alarm->period = 0;
    InsertAlarm(alarm, __OSTimeToSystemTime(time), handler);
    ASSERTLINE(389, OSCheckAlarmQueue());
    OSRestoreInterrupts(enabled);
}
...
void OSCancelAlarms(u32 tag) {
    ...
    ASSERTMSGLINE(614, tag != 0, "OSCancelAlarms(): invalid tag. (this tag is used by the operating system.)");
    ASSERTMSGLINE(615, tag != -1, "OSCancelAlarms(): invalid tag. (this tag is used by the operating system.)");
    ...
    enabled = OSDisableInterrupts();
    ASSERTLINE(623, OSCheckAlarmQueue());
    ...
    ASSERTLINE(633, OSCheckAlarmQueue());
    OSRestoreInterrupts(enabled);
}

static BOOL OnReset(BOOL final, u32 arg1) {
    OSAlarm* alarm;
    OSAlarm* next;
...
void OSSetAlarmUserData(OSAlarm* alarm, void* userData) {
    ASSERTMSGLINE(676, alarm, "NULL pointer is detected at %s", __FUNCTION__);
    alarm->userData = userData;
}

void* OSGetAlarmUserData(OSAlarm* alarm) {
    ASSERTMSGLINE(683, alarm, "NULL pointer is detected at %s", __FUNCTION__);
    return alarm->userData;
}

void __OSSetInternalAlarmUserData(OSAlarm* alarm, void* userData) {
    OSSetAlarmUserData(alarm, userData);
    alarm->tag = -1;
}
