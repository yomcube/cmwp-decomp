...
OSErrorHandler __OSErrorTable[__OS_EXCEPTION_MAX];
...
OSErrorHandler OSSetErrorHandler(OSError error, OSErrorHandler handler) {
    ...
    ASSERTMSGLINE(215, error < __OS_EXCEPTION_MAX, "OSSetErrorHandler(): unknown error.");
    ...
    if (error == __OS_EXCEPTION_FLOATING_POINT_EXCEPTION) {
        ...
        if (handler) {
            for (thread = __OSActiveThreadQueue.head; thread; thread = thread->linkActive.next) {
                ...
                thread->context.fpscr &= ~(FPSCR_VXVC | FPSCR_VXIMZ | FPSCR_VXZDZ | FPSCR_VXIDI | FPSCR_VXISI | FPSCR_VXSNAN | FPSCR_VXSOFT |
        ...
        fpscr &= ~(FPSCR_VXVC | FPSCR_VXIMZ | FPSCR_VXZDZ | FPSCR_VXIDI | FPSCR_VXISI | FPSCR_VXSNAN | FPSCR_VXSOFT | FPSCR_VXSQRT | FPSCR_VXCVI |
                   FPSCR_XX | FPSCR_ZX | FPSCR_UX | FPSCR_OX | FPSCR_FX | FPSCR_FI);
...
void __OSUnhandledException(__OSException exception, OSContext* context, u32 dsisr, u32 dar) {
    ...
    if (!(context->srr1 & MSR_RI)) {
        ...
        if (exception == __OS_EXCEPTION_PROGRAM && (context->srr1 & (0x80000000 >> 11)) &&
            ...
            if (__OSFPUContext == context) {
                ...
                context->fpscr &= ~(FPSCR_VXVC | FPSCR_VXIMZ | FPSCR_VXZDZ | FPSCR_VXIDI | FPSCR_VXISI | FPSCR_VXSNAN | FPSCR_VXSOFT | FPSCR_VXSQRT |
    ...
    switch (exception) {
        ...
        case __OS_EXCEPTION_MEMORY_PROTECTION: {
            OSReport("\n");
            OSReport("AI DMA Address =   0x%04x%04x\n", __DSPRegs[DSP_DMA_START_ADDR_H], __DSPRegs[DSP_DMA_START_ADDR_L]);
            OSReport("ARAM DMA Address = 0x%04x%04x\n", __DSPRegs[DSP_AR_DMA_MMADDR_H], __DSPRegs[DSP_AR_DMA_MMADDR_L]);
            OSReport("DI DMA Address =   0x%08x\n", __DIRegs[DSP_CONTROL_STATUS]);
            break;
        }
    }
