...
void __OSInterruptInit() {
    ...
    *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_PREVIOUS_INTERRUPT) = 0;
    *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_CURRENT_INTERRUPT) = 0;

    __PIRegs[PI_INTERRUPT_MASK] = 0xF0;
    __ACRRegs[HW_PPCIRQMASK] = 0x40000000;
...
#if DEBUG
    __PIRegs[PI_INTERRUPT_CAUSE] = 1;
    __OSUnmaskInterrupts(OS_INTERRUPTMASK_PI_ERROR);
#endif
}
...
static u32 SetInterruptMask(OSInterruptMask mask, OSInterruptMask current) {
    ...
    switch (__cntlzw(mask)) {
        ...
        case __OS_INTERRUPT_MEM_ADDRESS: {
            ...
            __MEMRegs[MEM_INT_ENBL] = (u16)reg;
        ...
        case __OS_INTERRUPT_EXI_0_EXT: {
            reg = __EXIRegs[EXI_0_CHAN_PARAM];
            reg &= ~0x2C0F;
        ...
        case __OS_INTERRUPT_EXI_1_EXT: {
            reg = __EXIRegs[EXI_1_CHAN_PARAM];
            reg &= ~0xC0F;
        ...
        case __OS_INTERRUPT_EXI_2_TC: {
            ...
            __EXIRegs[EXI_2_CHAN_PARAM] = reg;
        ...
        case __OS_INTERRUPT_PI_ACR: {
            ...
            __PIRegs[PI_INTERRUPT_MASK] = reg;
...
OSInterruptMask OSGetInterruptMask() {
    return *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_CURRENT_INTERRUPT);
}
...
OSInterruptMask OSSetInterruptMask(OSInterruptMask local) {
    ...
    global = *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_PREVIOUS_INTERRUPT);
    prev = *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_CURRENT_INTERRUPT);
    mask = (global | prev) ^ local;
    *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_CURRENT_INTERRUPT) = local;
    while (mask) {
...
OSInterruptMask __OSMaskInterrupts(OSInterruptMask global) {
    ...
    enabled = OSDisableInterrupts();
    prev = *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_PREVIOUS_INTERRUPT);
    local = *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_CURRENT_INTERRUPT);
    mask = ~(prev | local) & global;
    global |= prev;
    *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_PREVIOUS_INTERRUPT) = global;
    while (mask) {
        mask = SetInterruptMask(mask, global | local);
    }
    OSRestoreInterrupts(enabled);
...
void __OSDispatchInterrupt(__OSException exception, OSContext* context) {
    ...
    mask = __PIRegs[PI_INTERRUPT_MASK];
    ...
    unmasked = cause & ~(*(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_PREVIOUS_INTERRUPT) |
                         *(OSInterruptMask*)OSPhysicalToCached(OS_ADDR_CURRENT_INTERRUPT));
    if (unmasked) {
