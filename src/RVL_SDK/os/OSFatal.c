...
static void ConfigureVideo(u16 xfbW, u16 xfbH) {
    ...
    switch (VIGetTvFormat()) {
        case VI_NTSC:
        case VI_MPAL: {
            if (__VIRegs[VI_CLOCK_SELECT] & VI_CLOCK_SELECT_54MHZ) {
        ...
        case VI_EURGB60: {
            if (__VIRegs[VI_CLOCK_SELECT] & VI_CLOCK_SELECT_54MHZ) {
                mode.viTVmode = VI_TVMODE_EURGB60_PROG;
                mode.viYOrigin = 0;
                mode.xFBmode = VI_XFBMODE_SF;
            } else {
...
void OSFatal(GXColor fg, GXColor bg, const char* msg) {
    ...
    do {
    } while ((__EXIRegs[EXI_CONTROL] & EXI_CONTROL_TSTART) == 1);

    __OSSetExceptionHandler(8, &OSDefaultExceptionHandler);
    GXAbortFrame();
    OSSetArenaLo((void*)0x81400000);
    if (!bootInfo->FSTLocation) {
        OSSetArenaHi((void*)*(u32*)OSPhysicalToCached(OS_ADDR_AVAILABLE_MEM1_END));
    } else {
...
static void Halt() {
    ...
    fp = &FatalParam;
    len = strlen(fp->msg) + 1;
    fp->msg = memmove(OSAllocFromArenaLo(len, DEFAULT_ALIGN), fp->msg, len);

    fontData = OSAllocFromArenaLo(0xA1004, DEFAULT_ALIGN);
    OSLoadFont(fontData, OSGetArenaLo());

    xfb = OSAllocFromArenaLo(0x96000, DEFAULT_ALIGN);
